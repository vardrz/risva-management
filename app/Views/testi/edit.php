...
<div class="w-full px-4 py-2">
    <div class="flex justify-between pb-2">
        <h1 class="font-bold text-3xl items-center">Edit Testimoni</h1>
        <p></p>
    ...
    <div class="container mt-4 grid grid-cols-1 md:grid-cols-2 gap-5">
        <div class="card-body relative overflow-x-auto shadow-md border">
            <form action="<?= base_url('admin/testi/update/') . $data['id_testi']; ?>" method="post" enctype="multipart/form-data">
                <div class="mb-5">
                    <label for="pesan">Pesan</label>
                    <textarea rows="7" name="pesan" id="pesan" placeholder="Isi pesan ..." class="block px-2.5 pb-2.5 pt-4 mt-2 w-full resize-y text-sm text-gray-900 bg-transparent rounded-sm border appearance-none peer <?= isset(session()->get('validator')['pesan']) ? 'border-red-500' : 'border-[#B7B7B7]'; ?>"><?= $data['pesan']; ?></textarea>
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="nama" id="nama" value="<?= $data['nama']; ?>" placeholder="Nama" class="block px-2.5 pb-2.5 pt-4 mt-2 w-full resize-y text-sm text-gray-900 bg-transparent rounded-sm border appearance-none peer <?= isset(session()->get('validator')['nama']) ? 'border-red-500' : 'border-[#B7B7B7]'; ?>"/>
